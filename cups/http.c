...
httpClearFields(http_t *http)		/* I - Connection to server */
{
  if (http)
  {
    // 07/22/2016 Mopria-notice: preserve User Agent
    char buf[HTTP_MAX_VALUE];
    
    memcpy(buf, http->fields[HTTP_FIELD_USER_AGENT], HTTP_MAX_VALUE);
    memset(http->fields, 0, sizeof(http->fields));

    // 07/22/2016 Mopria-notice: restore User Agent
    if (buf[0]) httpSetField(http, HTTP_FIELD_USER_AGENT, buf);
...
#  ifdef HAVE_LIBSSL
  ...
  // 07/22/2016 Mopria-notice: disabling SSLv3
  SSL_CTX_set_options(context, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 ); /* Only use TLS */
